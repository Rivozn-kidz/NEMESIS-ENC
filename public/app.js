...
function validateDomains(domainString) {
  ...
  if (uniqueDomains.length !== domains.length) {
    throw new Error("Nemesis says,Duplicate domains found in Domain Lock");
  }
...
form.onsubmit = async function(event) {
  ...
  try {
    ...
    if (!javascriptCode.value.trim()) {
      throw new Error("Nemesis says,Please enter JavaScript code to obfuscate");
    }
    ...
    if (!response.ok) {
      throw new Error(result.error || "Nemesis says,Obfuscation failed");
    }
    ...
    showToast(`Nemesis says, Obfuscation successful (${currentSecurityLevel.toUpperCase()} level)`);
...
copyBtn.onclick = async function() {
  try {
    ...
    showToast("Nemesis says, Code copied to clipboard!");
  } catch (error) {
    showError("Nemesis says,Failed to copy: " + error.message);
  }
};
...
downloadBtn.onclick = function() {
  ...
  showToast("Nemesis says, Download started!");
};
